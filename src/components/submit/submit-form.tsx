...
export function SubmitForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (error) {
      // RLS rejects the insert once an admin closes submissions — the row-level
      // message is opaque, so say what actually happened.
      const closed = error.code === "42501" || /row-level security/i.test(error.message);
      toast.error(
        closed ? "Submissions have just been closed. Please check back later." : error.message
      );
      return;
    }
    setSubmitted(true);
