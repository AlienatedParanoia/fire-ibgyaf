...
// self-check parses.
// forked from design-sync lib/bundle.mjs — add process shim banner so Next.js + Supabase deps
// don't throw "process is not defined" in the browser preview sandbox.
...
// Node.js globals polyfill injected before the IIFE so Next.js + Supabase imports
// don't throw ReferenceError: process is not defined in the headless browser.
const PROCESS_SHIM = 'var process={env:{NODE_ENV:"production"},browser:true,version:"v18.0.0",versions:{},nextTick:function(f){setTimeout(f,0)}};';
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig }) {
  ...
  try {
    buildResult = await build({
      ...
      // Process shim: Next.js and Supabase reference process.env.* in their
      // module bodies. The banner runs before the IIFE, making process available.
